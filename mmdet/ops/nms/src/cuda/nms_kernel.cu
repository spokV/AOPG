...
__global__ void nms_kernel(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    const int col_blocks = (n_boxes + threadsPerBlock - 1) / threadsPerBlock;//THCCeilDiv(n_boxes, threadsPerBlock);
...
at::Tensor nms_cuda_forward(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const int col_blocks = (boxes_num + threadsPerBlock - 1) / threadsPerBlock;//THCCeilDiv(boxes_num, threadsPerBlock);
  ...
  dim3 blocks((boxes_num + threadsPerBlock - 1) / threadsPerBlock,//THCCeilDiv(boxes_num, threadsPerBlock),
              (boxes_num + threadsPerBlock - 1) / threadsPerBlock);//THCCeilDiv(boxes_num, threadsPerBlock));
